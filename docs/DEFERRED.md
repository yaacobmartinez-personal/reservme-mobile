...
| # | Item | Phase |
|---|---|---|
| D18 | **Verify onboarding on a device.** The flow is built and unit- and widget-tested, but O0–O7 have still not been walked on hardware. The render failure that blocked it is **not an app bug** — a stock `flutter create` counter app fails to paint on the same emulator, and the app's window comes up with `surface=[0,0][0,0]` while its frame is the full `[0,0][1080,2400]`, which is below anything Dart controls. The cause is the host: 7.7 GB of RAM with ~0.4 GB free once a Gradle daemon has taken 1.2 GB, against an AVD that asks for 2 GB. Under that pressure the emulator's own Qt/OpenGL UI crashes on startup (`Software OpenGL failed`, then a crash-consent dialog that blocks boot), and a headless run with less RAM boots but thrashes until `adb shell` stops answering. Neither `--no-enable-impeller` nor a config-change kick makes any difference. **To retry:** stop the Gradle daemons first (`android/gradlew --stop`), close other memory hogs, then boot the emulator — or use a physical device, which sidesteps the whole thing. | before merging 3b |
