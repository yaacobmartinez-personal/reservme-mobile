...
| D25 | **No impersonation in the app.** The web console can open a tenant's real dashboard as them; it works by setting a browser cookie on the app host, which has no meaning to a bearer-token client. An admin who needs to see what a venue sees uses the tenant page, or asks the owner. If it is ever needed, it wants a short-lived, audited bearer token scoped to one venue — a new design, not a port. | if asked |
